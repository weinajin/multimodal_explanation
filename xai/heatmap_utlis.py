...
def build_parser():
    ...
    parser.add_argument('--model_file', type=str,
                        default='/local-scratch/maskedname/BRATS_IDH/log/model_0920/fold_1_epoch_46.pth',
        ...
        for i in range(1, columns * rows + 1):
            ...
            if i <= columns:
                ...
                if self.hm_mix_vis:
                    ...
                    scaled_hm = normalize_scale(hm, self.outlier_perc) # normalize the full modality
...
def generate_mrivideo(video_name, exist_ok = True, subfolder = None, ffmpeg = True, dir = '/local-scratch/maskedname/trained_model/BRATS20_HGG/heatmaps/get_hm_fold_1/videos', png_to_video=False,  **kwargs):
    if subfolder:
...
# mri = '/Users/root202/OneDrive - maskedname/project/brain/heatmap_eval/heatmap_data/BraTS19_TCIA03_474_1/BraTS19_TCIA03_474_1_t2.nii.gz'
# path = '/Users/root202/OneDrive - maskedname/project/brain/heatmap_eval/heatmap_data/'
...
def IoU(seg, bn_hms):
    ...
    for hm in bn_hms:
        hm = hm.astype(int).reshape(-1)
        #         jaccard_score(ed.reshape(-1), ed.reshape(-1))
        iou_score['wt'].append(jaccard_score(wt, hm))
        iou_score['tc'].append(jaccard_score(tc, hm))
        iou_score['et'].append(jaccard_score(et, hm))
        iou_score['ed'].append(jaccard_score(ed, hm))
        iou_score['ncr'].append(jaccard_score(ncr, hm))
    return iou_score
...
def tumor_portion(seg, hms, ABS=False):
    ...
    if np.unique(hms).shape[0] == 2:
        ...
        for hm in hms:
            wt_portion = hm[wt == 1].sum() / hm.sum()
            tc_portion = hm[tc == 1].sum() / hm.sum()
            tumor_region_precetage['wt_portion'].append(wt_portion)
            tumor_region_precetage['tc_portion'].append(tc_portion)
...
if __name__ == '__main__':
    ...
    data_root = '/scratch/maskedname/dld_data/brats2020/MICCAI_BraTS2020_TrainingData/'#'/scratch/maskedname/dld_data/brats2020/MICCAI_BraTS_2020_Data_Training/'
    brats_path = os.path.join(data_root, 'all')
    # bgmask_path = '/local-scratch/maskedname/dld_data/brainmaskBRATS19'
    bgmask_path = '/scratch/maskedname/dld_data/brainmaskBRATS20'
    # bgmask_path = '/local-scratch/maskedname/dld_data/tmp'
    # bgmask_path = '/scratch/maskedname/dld_data/brainmaskBRATS19'
