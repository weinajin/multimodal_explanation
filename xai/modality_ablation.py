...
def generate_ablated_dataset(modalities = ["t1", "t1ce", "t2", "flair"], ablation_mode = 'allzero'):
    ...
    :return:
    """
    data_root = "/local-scratch/maskedname/dld_data/brats2020/MICCAI_BraTS2020_TrainingData/all_tmp"
...
def modality_shapley(config, ablated_image_folder, csv_save_dir = "/local-scratch/maskedname/BRATS_IDH/log/mod_shapley"):
    """
    Modality ablation experiment. Generate and save the ablated brats images
    Generate dataset with
    Save in the directory: Path(brats_path).parent / "ablated_brats", and can be loaded with the script:
    ...
    for modality_selection in N_sets:
        test(config, timestamp = False, ablated_image_folder = ablated_image_folder, save_dir = csv_save_dir, modality_selection= modality_selection)

def shapley_result_csv(fold = 1, root = '/local-scratch/maskedname/BRATS_IDH/log/mod_shapley/test/', modalities= ["t1", "t1ce", "t2", "flair"]):
    """
...
def get_shapley(csv_filename, modalities = ["t1", "t1ce", "t2", "flair"]):
    ...
    N_sets = list(itertools.product([0,1],repeat = 4)) # set of all_combinations
    for s in N_sets:
...
def compute_mfsi(hm_result_csv_root, gt_csv_path, modalities= ["t1", "t1ce", "t2", "flair"], msfi_save_dir = 'msfi_featshapley'):#, corr_name = "pearson"):
    fold_dict = {}
    # if corr_name == 'pearson':
    #     corr_method = stats.pearsonr
    # else:
...
def aggregate_msfi(target_folder = "msfi", col_name = 'msfi'):
    """
    ...
    for fold in range(1, 6):
        root = '/local-scratch/maskedname/trained_model/BRATS20_HGG/heatmaps/fold_{}/get_hm_fold_{}/{}/'.format(fold, fold, target_folder)
    ...
    for method in method_fl:
        ...
        save_dir = Path('/local-scratch/maskedname/trained_model/BRATS20_HGG/heatmaps/{}'.format(target_folder))
